...
function showStudents_bca(){
	
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM registration where `course`='15'";
	$stmt= $mysqli->query($query);
	return $stmt;
	
}	

function showStudents_bba(){
	
	$db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "SELECT * FROM registration where `course`='16'";
	$stmt= $mysqli->query($query);
	return $stmt;
...
function register($cshort,$cfull,$roll_no,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
				   $pyear1,$pyear2,$session){
 			          
 			        $db = Database::getInstance();
		           	$mysqli = $db->getConnection();
		           	
		           //	echo $session;exit;
   $query = "INSERT INTO `registration` (`course`, `sem`,`roll_no`, `fname`, `mname`, `lname`, `gender`, `gname`, `ocp`,
                     `income`, `category`, `nationality`, `mobno`, `emailid`, `country`, `state`, `dist`, 
					 `padd`, `cadd`, `board`, `board1`,`roll`,`roll1`,`pyear`,`yop1`,`session`,regno) 
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			        $reg=rand();
			        $stmt= $mysqli->prepare($query);
			        if(false===$stmt){
			...
			$stmt->bind_param('sssssssssssssssssssssssssss',
		         	$cshort,$cfull,$roll_no,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,$nation,$mobno,
...
function edit_std($cshort,$cfull,$session,$roll_no,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
				   $pyear1,$pyear2,$id){
  // echo $id;exit;
    $db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "update registration set course=?,sem=?,session=?,roll_no=?,fname=?,mname=?,lname=?,gender=?,gname=?,ocp=?
              , income=?,category=?,nationality=?,mobno=?,emailid=?,country=?,state=?,dist=?
         	 ,padd=?,cadd=?,board=?,board1=?,roll=?,roll1=?,pyear=?,yop1=? where id=?" ;
    //echo $query;
	$stmt= $mysqli->prepare($query);
	if(false===$stmt){
	...
	$rc=$stmt->bind_param('ssssssssssssssssssssssssssi',$cshort,$cfull,$session,$roll_no,$fname,$mname,$lname,$gender,$gname,$ocp,$income,$category,
                  $nation,$mobno,$email,$country,$state,$city,$padd,$cadd,$board1,$board2,$roll1,$roll2,
				   $pyear1,$pyear2,$id);
